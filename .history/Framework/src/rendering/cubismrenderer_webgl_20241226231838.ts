...
export class CubismClippingManager_WebGL extends CubismClippingManager<CubismClippingContext_WebGL> {
  ...
  public getMaskRenderTexture(): csmVector<WebGLFramebuffer> {
    ...
    if (this._maskTexture && this._maskTexture.textures != null) {
      ...
      for (let index = 0; index < this._renderTextureCount; index++) {
        ...
        if (!this._maskRenderTextures || this._maskRenderTextures.getSize() <= index) {
          console.error(`Invalid mask render texture at index ${index}`);
          return;
        }
  ...
  public setupClippingContext(
    ...
    console.log('clipCount:', usingClipCount);
...
const maskRenderTextures = this.getMaskRenderTexture();
if (!maskRenderTextures || maskRenderTextures.getSize() === 0) {
  console.error('Mask render textures are not properly initialized.');
  return;
}
this._currentMaskRenderTexture = maskRenderTextures.at(0);
      ...
      console.log('viewport:', this.gl.viewport);
...
export class CubismRenderer_WebGL extends CubismRenderer {
  ...
  public doDrawModel(): void {
    ...
    console.log('clippingManager :', this._clippingManager);
    if (this._clippingManager != null) {
      this.preDraw();

      if (this.isUsingHighPrecisionMask()) {
        console.log('high precision Mask :', this.isUsingHighPrecisionMask());
