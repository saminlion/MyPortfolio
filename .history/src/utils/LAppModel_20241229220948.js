...
*/
    CustomMotion(group, index, priority = 1, random=false, customCallback = null) {
        ...
        if (random)
        {
            if (group === 'Tap')
            {
                let maxIndex = this._modelSetting.getMotionCount(group);

                if (maxIndex <= 0)
                {
                    maxIndex = this._modelSetting.getMotionCount('TapBody');
                }

                console.log('Check Max:', maxIndex);

                newIndex =  Math.floor(Math.random() * (maxIndex - 0)) + 0;
            }
        }
    ...
    // 기본적으로 group과 newIndex를 조합하여 이름 생성
    let name = `${group}_${newIndex}`;
    let motion = this._motions.get(name); // 해당 모션 가져오기

    // group이 'Tap'이고, 해당 모션이 없는 경우 'TapBody'로 대체
    if (!motion && group === 'Tap') {
        console.warn(`Motion not found: ${name}. Checking for TapBody...`);
        const alternateName = `TapBody_${newIndex}`;
        motion = this._motions.get(alternateName);

        if (motion) {
            console.log(`Using alternate motion: ${alternateName}`);
            name = alternateName; // 실행할 모션 이름 변경
        }
    }

    // 모션이 여전히 없는 경우 경고 출력 후 종료
    if (!motion) {
        console.warn(`No valid motion found for group: ${group}, index: ${newIndex}`);
        return;
    }
