 ...
 */
    constructor(canvas, modelHomeDir, modelFileName, motionFileName, poseFileName,scale, xPos, yPos) {
        super();
        ...
        const render = () => {
            ...
            // if (deltaTime >= frameInterval) { // 프레임 간격을 초과한 경우에만 렌더링 실행
                ...
                if (this._model && this._textures.length > 0) {
                    if (this._pose != null) {
                        this._pose.updateParameters(this._model, deltaTime / 3000); // 포즈 업데이트 (초 단위로 전달)
                    }
        
                    this.motionManager.updateMotion(this._model, deltaTime / 3000); // 모션 업데이트 (초 단위로 전달)
