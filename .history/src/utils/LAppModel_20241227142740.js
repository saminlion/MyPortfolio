...
export class LAppModel extends CubismUserModel {
        ...
        try {
            ...
            if (idleCount > 0) {
                for (let i = 0; i < idleCount; i++) {
                    ...
                    if (!motion) {
                        console.warn(`Failed to load motion for: ${name}`);
                        continue;
                    }
                ...
                const motionFilePath = `${this.motionPath}/${this.motionName}`;
                ...
                if (!motion) {
                    ...
                    console.warn(`Failed to load motion for: ${this.motionName}}`);

                    return;
                }

                motion.setIsLoop(true);
                this._motions.set(this.motionName, motion);
            }

            if (this._motions.size === 0) {
                throw new Error('No idle motions were successfully loaded.');
            }
    ...
    startIdleAnimation() {
        if (this._motions.size === 0 || !this.motionManager) {
            console.error('No motions available or motionManager is not initialized.');
            return;
        }
    
        const idleMotion = this._motions.values().next().value; // 첫 번째 모션 사용
        if (!idleMotion) {
            console.error('Idle motion is undefined or not loaded.');
            return;
        }
