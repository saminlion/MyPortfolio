...
import { CubsimMotion } from 'live2d/motion/cubismmotion';
import { CubismMotionQueueManager } from 'live2d/motion/cubismmotionqueuemanager'
...
export class Live2DLoader {
    ...
    constructor(canvas, modelPath, motionPath, motionName, modelDirectory) {
        this.canvas = canvas;
        ...
        this.motionManager = new CubismMotionQueueManager();
    }
    ...
    async loadLive2DModel() {
        try {
            ...
            this.setupRenderer(modelSetting);
    ...
    async loadLive2DMotion(){
        try {
            const motionSettingPath = this.motionPath + this.motionName + ".motion3.json";
            const response = await fetch(motionSettingPath);
            console.log('motion response:', response);
            const arrayBuffer = await response.arrayBuffer();
            const motionSetting = new CubsimMotion();
            motionSetting.create(arrayBuffer, arrayBuffer.byteLength);
