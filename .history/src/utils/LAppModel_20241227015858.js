...
export class LAppModel extends CubismUserModel {
    ...
    async loadLive2DMotion() {
        try {
            ...
            for (let i = 0; i < this._modelSettingJson.getMotionCount('idle'); i++) {
                const name = `${'idle'}_${i}`;

                const motionFileName = this._modelSettingJson.getMotionFileName('idle', i);

                console.log('motionFileName:', motionFileName);
                ...
                if (motion != null) {
                    let fadeTime = this._modelSetting.getMotionFadeInTimeValue('idle', i);

                    if (fadeTime >= 0.0) {
                        motion.setFadeInTime(fadeTime);
                    }

                    fadeTime = this._modelSetting.getMotionFadeOutTimeValue('idle', i);
    ...
    startIdleAnimation() {
        try {
            ...
            const motionFileName = this._modelSettingJson.getMotionFileName('idle', 0);
            ...
            fetch(motionFilePath)
                ...
                .then((arrayBuffer) => {
                    const motion = this.loadMotion(arrayBuffer, arrayBuffer.byteLength, null);
                    if (motion) {

                        let fadeTime = this._modelSetting.getMotionFadeInTimeValue('idle', 0);

                        if (fadeTime >= 0.0) {
                            motion.setFadeInTime(fadeTime);
                        }
    
                        fadeTime = this._modelSetting.getMotionFadeOutTimeValue('idle', 0);
