 ...
 */
    ...
    async initialize() {
        try {
            ...
            // 7. 기본 모션 시작
            this.IdleMotion('Idle', this.groupIndex, 0);
  ...
  */
    ...
    IdleMotion(group, index, priority = 1) {
        ...
        this.motionManager.startMotionPriority(motion, true, priority); // 모션 실행
    }
     ...
     * @param {number} priority - 모션 실행 우선순위
     */
    CustomMotion(group, index, priority = 1) {
        const name = `${group}_${index}`; // 그룹과 인덱스를 조합하여 모션 이름 생성
        const motion = this._motions.get(name); // 모션 객체 가져오기
        ...
        this.motionManager.startMotionPriority(motion, true, priority, () => {
            console.log(`Custom motion finished: ${name}`);
            this.IdleMotion('Idle', 0, 2); // IdleMotion 실행
        }); // 모션 실행
    }
