...
export class LAppModel extends CubismUserModel {
     ...
     */
    constructor(canvas, modelHomeDir, modelFileName, motionFileName, poseFileName) {
        super();
    ...
    async loadRenderProjection() {
        ...
        // 모델의 스케일 및 화면 비율 계산
        const scale = 1.0; // 기본 스케일 값 (조정 가능)
  ...
  */
    async preloadMotions() {
        ...
        const motionGroup = 'Idle'; // 로드할 모션 그룹 이름
        const motionCount = this._modelSetting.getMotionCount(motionGroup); // 모션 그룹 내 모션 개수

        // 모든 모션 파일 비동기 로드
        const motionPromises = Array.from({ length: motionCount }, async (_, i) => {
            const motionFileName = this._modelSetting.getMotionFileName(motionGroup, i); // 모션 파일 이름 가져오기
            const motionPath = `${this.motionPath}${motionFileName}`; // 모션 파일 경로 생성
            const response = await fetch(motionPath);
            if (!response.ok) throw new Error(`Failed to load motion: ${response.statusText}`); // 로드 실패 처리
            const arrayBuffer = await response.arrayBuffer();

            // 모션 생성 및 초기화
            const motion = this.loadMotion(arrayBuffer, arrayBuffer.byteLength, `${motionGroup}_${i}`);
            let fadeTime = this._modelSetting.getMotionFadeInTimeValue(motionGroup, i);
            ...
            this._motions.set(`${motionGroup}_${i}`, motion); // 모션 저장
        });
        ...
        const render = () => {
            ...
            if (deltaTime >= frameInterval) { // 프레임 간격을 초과한 경우에만 렌더링 실행
                ...
                if (this._model && this._textures.length > 0) {
                    if (this._pose != null) {
                        this._pose.updateParameters(this._model, deltaTime / 1000); // 포즈 업데이트 (초 단위로 전달)
                    }
        
                    this.motionManager.updateMotion(this._model, deltaTime / 1000); // 모션 업데이트 (초 단위로 전달)
