...
export class Live2DLoader {
    ...
    async loadLive2DMotion() {
        try {
           ...
           //const motion = this.cubismModel.loadMotion(arrayBuffer, arrayBuffer.byteLength, this.motionName);
            ...
            const motionHandle = this.motionManager.startMotionPriority(motion, false, 2);
            if (motionHandle === -1) {
    ...
    async setupRenderer(modelSetting) {
        ...
        const scale = 2.0; // 모델의 스케일 값 설정 (큰 값 그대로 유지)
    ...
    updateModel(deltaTime) {
        ...
        try {
            ...
            if (!motionUpdated) {
                // 모션이 업데이트되지 않았을 경우 새로운 모션 시작
                console.warn('No motion updates were applied. Starting default motion.');
    
                // 기본 모션을 다시 시작 (모션 설정 필요 시 추가 구현)
                const defaultMotion = this.loadDefaultMotion();
                if (defaultMotion) {
                    this.motionManager.startMotionPriority(defaultMotion, false, 1);
                } else {
                    console.error('Failed to load or start the default motion.');
                }
            } else {
