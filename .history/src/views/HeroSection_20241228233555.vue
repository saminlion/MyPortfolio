    ...
    <div class="hero-content detail-layout">
      ...
      <div v-if="isDetailView" class="project-detail-container">
        <div class="project-detail">
          <h2>{{ reactiveCurrentProject?.name }}</h2>
          <img :src="reactiveCurrentProject?.imageUrl" alt="Project Image" class="project-image" />
          <p>{{ reactiveCurrentProject?.description }}</p>
          <ul v-if="reactiveCurrentProject?.tools?.length">
            <li v-for="tool in reactiveCurrentProject.tools" :key="tool">{{ tool }}</li>
          </ul>
        </div>
      </div>
    </div>
...
<script setup>
import { ref, watch, computed, onMounted, props } from 'vue';
...
const reactiveCurrentProject = ref(props.currentProject);
const reactiveIsDetailView = ref(props.isDetailView);
...
watch(
  ...
  (newProject) => {
    reactiveCurrentProject.value = newProject;

    console.log('New Project Loaded:', reactiveCurrentProject.value);
  }
);
...
watch(
  ...
  (detailView) => {
    ...
    reactiveIsDetailView.value = detailView;


    console.log('Detail View Status Changed:', reactiveIsDetailView.value);
  }
);
