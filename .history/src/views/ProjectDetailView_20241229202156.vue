...
function loadProjects(lang) {
  ...
  currentProject.value = projects.value.find((id) => {
    ...
    //82 10 001 //81 20 001
    console.log(`Find ID : ${String(id).slice(2,7)}, Compare ID: ${String(currentProjectId.value).slice(2, 7)}`);

    return String(id).slice(2, 7) === String(currentProjectId.value).slice(2, 7);
  }) || null;
}
