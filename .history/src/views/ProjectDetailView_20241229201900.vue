...
function loadProjects(lang) {
  ...
  currentProject.value = projects.value.find((id) => {
    // 비교 전 타입 일치 보장

    
    console.log(`Find ID : ${String(id).slice(0,5)}, Compare ID: ${String(currentProjectId.value).slice(2, 0)}`);

    return String(id).slice(0, 5) === String(currentProjectId.value).slice(0, 5);
  }) || null;
}
