<template>
  <section class="hero">
    ...
    <div class="hero-content detail-layout">
      ...
      <div v-show="currentPage.value === 'projects' || !animationCompleted" :class="{ dissolve: isDetailView }" @animationend="onDissolveComplete"
        class="canvas-box">
      </div>
...
watch(
  ...
  (newPage) => {
    ...
    if (canvasBox) {
      canvasBox.classList.add('show'); // show 클래스로 표시
    }
  } else {
    if (canvasBox) {
      canvasBox.classList.remove('show'); // show 클래스로 숨김
    }
  }
...
watch(isDetailView, (newVal) => {
  if (newVal) {
    ...
    live2DLoader.value.CustomMotion('TapBody', 0, 2, () => {
...
onMounted(() => {
  ...
  const canvasBox = document.querySelector('.canvas-box');
  if (currentPage.value === 'projects' && canvasBox) {
    canvasBox.classList.add('show'); // 페이지 로드 시 projects에서 보이도록 설정
  }
});
</script>
