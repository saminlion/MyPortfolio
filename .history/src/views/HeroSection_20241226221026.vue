...
watch(
    ...
    (newPage) => {
        ...
        loadLive2DModel(currentModel.value);
    }
);

// Live2D 모델 초기화
function loadLive2DModel(canvas, modelPath, modelFileName, motionPath, motionName) {
...
onMounted(() => {
    if (live2dCanvas.value) {
        const modelPath = currentModel.value.modelPath;
        const modelFileName = currentModel.value.modelFileName;
        const motionPath = currentModel.value.motionPath;
        const motionName = currentModel.value.motionName;

        loadLive2DModel(live2dCanvas.value, modelPath, modelFileName, motionPath, motionName);
