<template>
  ...
  const models = {
    home: { modelPath: '/Model/Chitose/', modelFileName: 'Chitose.model3.json', motionPath: '/Model/Chitose/', motionName: '00_idle', poseFileName: 'Chitose.pose3.json' },
    projects: { modelPath: '/Model/Haru/', modelFileName: 'Haru.model3.json', motionPath: '/Model/Haru/', motionName: 'haru_g_m15', poseFileName: 'Haru.pose3.json' },
    about: { modelPath: '/Model/tororo/', modelFileName: 'tororo.model3.json', motionPath: '/Model/tororo/', motionName: '00_idle' , poseFileName: 'tororo.pose3.json'},
  };
...
const currentModel = ref(models[currentPage.value] || models.home);
  ...
  function loadLive2DModel() {    
    ...
    //clearPreviousModel(); // 이전 모델 정리

    const { modelPath, modelFileName, motionPath, motionName, poseFileName } = currentModel.value;
    ...
    //console.log(`Loading Live2D model: ${modelPath}, ${modelFileName}, ${motionPath}, ${motionName}, ${poseFileName}`);
        console.log(`Loading Live2D model: ${modelPath}, ${modelFileName}, ${motionPath}, ${motionName}, ${poseFileName}`);
    ...
    try {
      ...
      live2DLoader.value = new LAppModel(canvas, modelPath, modelFileName, motionPath, motionName, poseFileName);
  ...
  function clearPreviousModel() {
    if (live2DLoader.value) {
        live2DLoader.value.destroy(); // 이전 모델의 리소스를 정리
        live2DLoader.value = null; // 참조 해제
    }

    if (live2dCanvas.value) {
        const gl = live2dCanvas.value.getContext('webgl');
        if (gl) {
            gl.clearColor(0.0, 0.0, 0.0, 0.0); // 캔버스 초기화
