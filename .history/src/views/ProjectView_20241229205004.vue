...
const projectStore = useProjectStore(); // Pinia 프로젝트 스토어

// 프로젝트 데이터를 저장할 상태
const projects = ref([]);
const selectedFilter = ref('all');

const techStacks = ['Unity3D', 'Unreal Engine', 'Winform', 'VR'];

// 필터와 연관된 키워드 목록
const filterKeywords = {
  VR: ['HTC Vive', 'VR CHAT', 'Oculus Rift', 'Samsung Odyssey', 'Virtual Reality', 'AR', 'Mixed Reality', 'Meta Quest', 'Pimax'],
  'Unity3D': ['Unity3D', 'C#', 'Unity'],
  'Unreal Engine': ['Unreal Engine', 'Unreal Engine 4', 'Unreal Engine 5', 'UE4', 'UE5'],
  'Winform': ['Winform', 'Windows Forms'],
};
...
function applyFilter(filter) {
  ...
  if (filter === 'all') {
    filteredProjects.value = projects.value; // 모든 프로젝트 표시
  } else {
    const keywords = filterKeywords[filter] || [filter]; // 필터에 해당하는 키워드 목록 가져오기
...
function viewProject(id) {
  ...
  projectStore.setProject(id, selectedProject.value); // Pinia 업데이트
}
</script>
