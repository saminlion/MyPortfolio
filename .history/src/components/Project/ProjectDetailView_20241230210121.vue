<template>
  <div class="project-detail">
    ...
    <div class="s" v-if="Array.isArray(currentProject?.outcomes) && currentProject?.outcomes.length > 0">
      <div class="tab-content" v-if="activeTab === 'achive'">
        <!-- 프로젝트 성과 리스트 -->
        <div class="role-cards">
          <div v-for="(outcome, index) in currentProject?.outcomes" :key="index" class="role-card">
            <div class="role-icon">
              <i class="fas fa-tasks"></i>
            </div>
            <div class="role-content">
...
import YoutubeVideoPlayer from '../Project/YoutubeVideoPlayer.vue';
...
watch(
  ...
  (id) => {
    ...
    if (id != 0) {
      ...
      console.log('check Length:', currentProject.value.outcomes.length);
      ...
      // outcomes가 존재하고 비어 있지 않은 경우에만 성과 탭 추가
      if (Array.isArray(currentProject.value?.outcomes) && currentProject.value.outcomes.length > 0) {
        tabs.value.push({ id: 'achive', label: t('project.achieve') });
      }
    }
  }
);
...
.project-detail {
  padding: 2rem;
  background: var(--project-detail-background);
  /* 새로운 변수 적용 */
  color: var(--project-detail-text-color);
  /* 새로운 변수 적용 */
  border-radius: 8px;
