<template>
    <section id="contact" class="contact-section">
      ...
      <div class="contact-links">
        <p>
        Email: 
        <a @click="copyEmail()" href="javascript:void(0)">swh5246@gmail.com</a>
        <Toast v-if="copySuccess" class="notification">Copied to clipboard! </Toast>
      </p>
      <p>
        LinkedIn: 
        <a href="https://ca.linkedin.com/in/sa-min-hong-aab397108" target="_blank">Profile</a> | 
        Github: 
...
const toast = useToast();
  ...
  function copyEmail() {
  const email = 'swh5246@gmail.com';
  navigator.clipboard
    .writeText(email)
    .then(() => {
      // 복사 성공 시 알림 표시
      console.log('success copy email');
      copySuccess.value = true;
        show();
    })
    .catch((error) => {
      console.error('Failed to copy email:', error);
      toast.add({ severity: 'error', summary: 'Fail Copy Email', detail: 'Email Address Copy Failed', life: 3000 });
    });
}

const show = () => {
    console.log('check');
    toast.add({ severity: 'success', summary: 'Copy Email', detail: 'Email Address Copied', life: 3000 });
